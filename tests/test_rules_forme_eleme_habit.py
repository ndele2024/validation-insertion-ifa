...
from rules.forme_eleme_habit import NOMBRE_MAX_ENREGISTREMENTS, forme_eleme_habit_nombre_max


def _rows(n):
    return [{"une_code_ident": "UE1", "mes_no_seq": 1, "hab_no_stati": 1} for _ in range(n)]


def test_forme_eleme_habit_nombre_max_registered_as_layer_kind():
    ...
    rules = get_rules("forme_eleme_habit", kind=RuleKind.LAYER)
    assert any(r.name == "forme_eleme_habit_nombre_max" for r in rules)
    assert not any(
        r.name == "forme_eleme_habit_nombre_max"
        for r in get_rules("forme_eleme_habit", kind=RuleKind.VALIDATION)
    )


def test_aucune_forme_no_error(make_context):
    ctx = make_context("forme_eleme_habit", {}, all_rows=[])
    assert forme_eleme_habit_nombre_max(ctx) == []


def test_nombre_egal_au_maximum_no_error(make_context):
    rows = _rows(NOMBRE_MAX_ENREGISTREMENTS)
    ctx = make_context("forme_eleme_habit", {}, all_rows=rows)
    assert forme_eleme_habit_nombre_max(ctx) == []
...
def test_nombre_superieur_au_maximum_triggers_error(make_context):
    ...
    ctx = make_context("forme_eleme_habit", {}, all_rows=rows)
    issues = forme_eleme_habit_nombre_max(ctx)
    assert len(issues) == 1
    assert issues[0].code == "FORME_ELEME_HABIT_NOMBRE_MAX_ENREGISTREMENTS"
...
def test_nombre_tres_superieur_ne_produit_qu_une_seule_anomalie(make_context):
    ...
    rows = _rows(50)
    ctx = make_context("forme_eleme_habit", {}, all_rows=rows)
    assert len(forme_eleme_habit_nombre_max(ctx)) == 1
